...
function addMessage(m) {
    ...
    if (m.destruct_type === 'timed' && m.destruct_duration && m.timestamp && destructInfoSpan) {
        ...
        if (m.user === currentUser) {
            destructInfoSpan.textContent = ' (🔥 Burn after read for recipients)';
            // Add class for specific styling if sender views their own BAR message
            destructInfoSpan.classList.add('burn-for-recipient');
        } else { // Message is from another user
            destructInfoSpan.textContent = ' (🔥 Burn after read. Vanishing soon...)';
